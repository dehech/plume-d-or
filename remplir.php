            ...
            if($_POST['r1']=="client"){
              ...
              $r=mysqli_query($link,$q);
                if (mysqli_num_rows($r)==0){
                  $q="insert into client (`id_client`	, `nom`, `prenom`, `email`, `date_naissance`, `mdp`, `tel`, `adresse`) values (1, '$nom', '$prenom', '$email', $datn, '$pwd', '$tel', '$adr')";
                  $r=mysqli_query($link,$q);
                  ...
                  if (mysqli_num_rows($r)==0){
                    ...
                    $max_id_client = $row[0];
                    $q="insert into client values ('$max_id_client'+1, '$nom', '$prenom', '$email', $datn, '$pwd', '$tel', '$adr')";
                    $r=mysqli_query($link,$q);
                  header ("Location: inscription_réussie.php");
                  }else
              ...
              $q="select * from peintre";
              $r=mysqli_query($link,$q);
                if (mysqli_num_rows($r)==0){
                  $q="insert into peintre values (1, '$nom','$prenom','$email','$pwd','$tel','$adr')";
                  $r=mysqli_query($link,$q);
                  header ("Location: inscription_réussie.php");
                }else{
                  $q="select * from peintre where email='$email'";
                  $r=mysqli_query($link,$q);
                  if (mysqli_num_rows($r)==0){
                    $q="select max(id_peintre) from peintre";
                    $v=mysqli_query($link,$q);
                    $row = mysqli_fetch_array($v);
                    $max_id_peintre = $row[0];
                    $q="insert into peintre values ('$max_id_peintre'+1, '$nom','$prenom','$email', '$pwd','$tel','$adr')";
                    $r=mysqli_query($link,$q);
                    header ("Location: inscription_réussie.php");
                  }else
                    header ("Location: exist.php");
...
<div class="container">
    <form method="POST">
      <fieldset>
        ...
        <div class="row">
          <div class="col">
            <label class="radio-label">Client<input type="radio" name="r1" value="client"></label>
          </div>
          <div class="col">
            <label class="radio-label">Peintre<input type="radio" name="r1" value="peintre"></label>
          </div>
        </div>
        <div class="row">
          <div class="col">
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" name="test" placeholder="N'hésitez pas"></textarea>
        </div>
        <div class="row">
          <div class="col">
